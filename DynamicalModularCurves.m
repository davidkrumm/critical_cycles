...
PreimageMorphism_finite := function(f,m,P,i)	assert i ge 1 and i le m;
	// Ensure f has coefficients in a one-variable function field.
	Qt := FieldOfFractions(CoefficientRing(Parent(f))); // a one-variable function field
	a := Numerator(f);
	b := Denominator(f);
	a := Polynomial([Qt ! coeff : coeff in Coefficients(a)]);
	b := Polynomial([Qt ! coeff : coeff in Coefficients(b)]);
	...
	_,F,_,_ := Normalize(Iterate(f_copy,m) - (Qt ! P));
	AA := AffineSpace(Rationals(),2);
	g := Substitute(F,[AA.1,AA.2]);
	YmP := Curve(AA,g);
	// Construct the curve Y(i,P).	
	_,F,_,_ := Normalize(Iterate(f_copy,i) - (Qt ! P));
	g := Substitute(F,[AA.1,AA.2]);
	YiP := Curve(AA,g);
